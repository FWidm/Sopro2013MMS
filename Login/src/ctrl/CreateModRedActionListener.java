...
public class CreateModRedActionListener implements ActionListener {
	...
	public void refreshMenu(String itemValue) {
		...
		if (bean.getExRules() == null) {
			...
			modMan.setAjax(true);
			modMan.setUpdate(CreateModRedBean.UPDATE_AJAX);
			modMan.setValue(CreateModRedBean.MODMANUAL);
			...
			MenuModel model = new DefaultMenuModel();
			Submenu submenu = new Submenu();
			submenu.setLabel(CreateModRedBean.MODULE);
			model.addSubmenu(submenu);
		...
		/*
		 * else { MenuModel backModel = new DefaultMenuModel(); Submenu
		 * backSubmenu = new Submenu(); backSubmenu.setLabel("Zur\u00FCck zu:");
		 * MenuItem exRules = new MenuItem(); exRules.setAjax(true);
		 * exRules.setUpdate(CreateModRedBean.UPDATE_AJAX);
		 * exRules.setValue(CreateModRedBean.PRUEFORDNUNG);
		 * exRules.addActionListener(back);
		 * backSubmenu.getChildren().add(exRules); MenuItem modMan = new
		 * MenuItem(); modMan.setAjax(true);
		 * modMan.setUpdate(CreateModRedBean.UPDATE_AJAX);
		 * modMan.setValue(CreateModRedBean.MODMANUAL);
		 * modMan.addActionListener(back);
		 * backSubmenu.getChildren().add(modMan); MenuItem module = new
		 * MenuItem(); module.setAjax(true);
		 * module.setUpdate(CreateModRedBean.UPDATE_AJAX);
		 * module.setValue(CreateModRedBean.MODULE); module.addActionListener(back);
		 * backSubmenu.getChildren().add(module);
		 * backModel.addSubmenu(backSubmenu); bean.setBackModel(backModel);
		 * 
		 * MenuModel model = new DefaultMenuModel(); Submenu submenu = new
		 * Submenu(); submenu.setLabel(CreateModRedBean.FAECHER);
		 * model.addSubmenu(submenu);
		 * 
		 * for(int i = 0; i < bean.getSubjectList().size(); i++) { MenuItem m =
		 * new MenuItem(); m.setAjax(true);
		 * m.setUpdate(CreateModRedBean.UPDATE_AJAX);
		 * m.setValue(bean.getSubjectList().get(i).getSubTitle());
		 * m.addActionListener(this); submenu.getChildren().add(m); }
		 * 
		 * bean.setModel(model);
		 * 
		 ...
		 * bean.getModule());
		 * 
		 * //for PDFBox DownloadBean download = findBean("DownloadBean");

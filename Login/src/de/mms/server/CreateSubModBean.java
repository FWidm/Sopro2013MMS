...
import de.mms.ctrl.CreateSubModActionListener;
...
@ManagedBean(name = "CreateSubModBean")
@SessionScoped
public class CreateSubModBean {
	...
	public CreateSubModBean() {
		...
		for (int i = 0; i < exRulesList.size(); i++) {
			...
			m.addActionListener(new CreateSubModActionListener());
	...
	public void handleModule(Module mod) {
		...
		for (int i = 0; i < userRights.size(); i++) {
			...
			if (userRights.get(i).equals(mod.getModTitle())) {
				found = true;
				break;
			}
		}

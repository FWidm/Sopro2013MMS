...
import de.mms.server.TemplateBean;
...
public class TemplateActionListener implements ActionListener {
	...
	public void refreshMenu(String itemValue) {
		...
		if (bean.getExRules() == null) {
			...
			ExRules exRule = DBExRules.loadExRules(itemValue);

			DownloadBean download = findBean("DownloadBean");
			download.setDownloadExRule(exRule);

			bean.handleExRule(exRule);
			...
			ModManual modManual = DBModManual.loadModManual(itemValue);

			DownloadBean download = findBean("DownloadBean");
			download.setDownloadModMan(modManual);

			bean.handleModManual(modManual);
			...
			module.setAjax(true);
			module.setUpdate(TemplateBean.UPDATE_AJAX);
